...
class FermentsSpiral(Scene):
    def construct(self):
        title = Tex(r"Polar Sketch of")
        title.to_edge(UP)
        polar_curve=Tex(r"Ferment's Spiral")
        polar_curve.next_to(title,DOWN,buff = 0.3)
        formula = MathTex(r"r^2=a^2\theta")
        ...
        self.wait(2)
        self.play(AnimationGroup(a.animate.set_value(-7),lag_ratio=0.1),run_time=5,rate_func=linear)
        self.wait(2)
