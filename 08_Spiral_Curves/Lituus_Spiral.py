...
class LituusSpiral(Scene):
    def construct(self):
        title = Tex(r"Polar Sketch of")
        title.to_edge(UP)
        polar_curve=Tex(r"Lituus Spiral")
        polar_curve.next_to(title,DOWN,buff = 0.3)
        formula = MathTex(r"r^2=\dfrac{a^2}{\theta}")
        ...
        Spiral = always_redraw(lambda: plane.plot_polar_graph(lambda theta: np.sqrt(a.get_value()**2 / theta),theta_range=[0.1,angle.get_value()],color=ORANGE))
        ...
        self.wait(2)
        self.play(AnimationGroup(a.animate.set_value(-70),lag_ratio=0.1),run_time=5,rate_func=linear)
        self.wait(2)
